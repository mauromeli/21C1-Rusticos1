...
#[allow(dead_code)]
pub fn decode(bytes: &[u8], start: usize) -> (TypeData, usize) {
    match bytes[start] as char{
        '+' => {
            let (string, final_index) = parse(bytes, start+1);
            (TypeData::String(string), final_index)
        }
        '-' => {
            let (error, final_index) = parse(bytes, start+1);
            (TypeData::Error(error), final_index)
        }
        ':' => {
            let (integer, final_index) = parse(bytes, start+1);
            (TypeData::Integer(integer.parse::<i64>().unwrap()), final_index)
        }
        ...
        '$' => {
            ...
            (TypeData::BulkString(bulk), length+final_index+CRLF)
        }
        ...
        '*' => {
            ...
            for _ in 0..length {
                let (data, final_pos) = decode(bytes, final_index);
                array.push(data);
                final_index = final_pos;
            }
            (TypeData::Array(array), final_index)
        }
        _ => {
            (TypeData::Nil, 0)
        }
...
fn test_decode_string(){
    let bytes = "+OK\r\n";
    assert_eq!(decode(bytes.as_bytes(), 0).0,
