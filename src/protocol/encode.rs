...
pub fn encode(data: &TypeData) -> Vec<u8> {
    match data {
        ...
        TypeData::Error(ref error) => {
            let bytes = ["-".to_string().as_bytes(), error.as_bytes(), "\r\n".as_bytes()].concat();
            bytes
        }
        TypeData::Integer(ref int) => {
            let bytes = [":".to_string().as_bytes(), int.to_string().as_bytes(), "\r\n".as_bytes()].concat();
            bytes
        }
        TypeData::BulkString(ref bulk) => {
            let bytes = ["$".to_string().as_bytes(), bulk.len().to_string().as_bytes(), "\r\n".as_bytes(),
                bulk.as_bytes(), "\r\n".as_bytes()].concat();
            bytes
        }
